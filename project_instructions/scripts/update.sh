...
ARCHIVEBIN_DIR="./archivebin"
BACKUP_DIR="$ARCHIVEBIN_DIR/backup_$(date +%Y%m%d_%H%M%S)"
...
create_backup() {
    if [ -f "project_instructions/README.md" ] || [ -d "project_instructions" ]; then
        ...
        # Create archivebin directory if it doesn't exist
        if [ ! -d "$ARCHIVEBIN_DIR" ]; then
            print_status "Creating archivebin directory: $ARCHIVEBIN_DIR"
            mkdir -p "$ARCHIVEBIN_DIR"
        fi

        # Create backup directory inside archivebin
        mkdir -p "$BACKUP_DIR"
...
main() {
    ...
    if [ -d "$BACKUP_DIR" ]; then
        ...
        print_status "Backups are stored in the archivebin directory for organized file management."
    fi
